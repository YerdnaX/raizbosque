...
import { TourPrincipal } from '@/features/onboarding/components/TourPrincipal';
import { useOnboardingPrincipal } from '@/features/onboarding/hooks/useOnboardingPrincipal';
import { ALTURA_BASE_TAB_BAR } from '@/features/onboarding/constants/pasosOnboarding';

export default function TabsLayout() {
    const insets = useSafeAreaInsets();
    const { t } = useIdioma();
    const onboarding = useOnboardingPrincipal();
    ...
    return (
        <>
            <Tabs
                screenOptions={{
                    ...
                    tabBarStyle: {
                        ...
                        borderTopWidth: 1,
                        height: ALTURA_BASE_TAB_BAR + insets.bottom,
            ...
            {onboarding.estaListo && (
                <TourPrincipal
                    visible={onboarding.debeMostrarse}
                    pasoActual={onboarding.pasoActual}
                    totalPasos={onboarding.totalPasos}
                    onSiguiente={onboarding.siguiente}
                    onAnterior={onboarding.anterior}
                    onOmitir={onboarding.omitir}
                />
            )}
        </>
    );
